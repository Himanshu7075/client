...
const DateSelect = ({dateTime, id}) => {
    ...
    const onBookHandler = () => {
        ...
        navigate(`/movie/${id}/${encodeURIComponent(selected)}`)
        scrollTo(0,0)
    }
    ...
    <div id='dateSelect' className='pt-30'>
      <div className='flex flex-col md:flex-row items-center justify-between gap-10 relative p-8 bg-primary/10 border-primary/20 rounded-lg'>
        ...
        <button onClick={onBookHandler}
            className='bg-primary text-white px-8 py-2 mt-6 rounded hover:bg-primary/90 transition cursor-pointer'>Book Now</button>
      </div>
    </div>
  );
}
